...
class PsychoJudgmentPreview extends StatefulWidget {
  const PsychoJudgmentPreview({required this.register, super.key});
  final Register register;
  @override
  State<PsychoJudgmentPreview> createState() => _PsychoJudgmentPreviewState();
}

class _PsychoJudgmentPreviewState extends State<PsychoJudgmentPreview> {
  late String pathFile;
  late KartaKz kartaKzPsycho;

  Future<KartaKz> futureKz(String number) async {
    ...
    var kartKz = KartaKz.fromJson(documents.first.map);
  ...
  void createFiles(KartaKz kartaKz, String filePath, Patient patient, Firm? firm) {
    createKzFile(kartaKz, filePath, patient);
    for (var element in kartaKz.judgments) {
      crateJudgmentPdf(element, filePath, patient, firm);
    }
  }

  Future<void> createKzFile(KartaKz kartaKz, String path, Patient patient) async {
    ...
    var file = getKzFileWithPath(path: path, number: kartaKz.number, type: DocumentType.psycho);
    await file.writeAsBytes(await pdf.save());
  }

  Future<void> crateJudgmentPdf(Judgment judgment, String path, Patient patient, Firm? firm) async {
    var font = await rootBundle.load("fonts/Lato-Regular.ttf");
    var myTheme = pw.ThemeData.withFont(
      base: pw.Font.ttf(await rootBundle.load("fonts/Lato-Regular.ttf")),
      bold: pw.Font.ttf(await rootBundle.load("fonts/Lato-Bold.ttf")),
